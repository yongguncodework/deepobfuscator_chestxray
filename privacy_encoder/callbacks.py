...
class Reconstruction(keras.callbacks.Callback):
    ...
    def load_image2(self, file_path):
        ...
        x = keras.preprocessing.image.load_img(
            file_path, grayscale=False, target_size=[128, 128, 1]
        )
...
# class ReconstructionByClass2(Reconstruction):
#     def __init__(
#             self,
#             encoder,
#             decoder,
...
#         # samples = random.sample(files, k=self.n_images)
#         samples = files[:5]
#         self.neg_images = [self.load_image2(sample) for sample in samples]
#
#         # Get test set samples
#         files = [os.path.join(image_dir, i) for i in pos_class_df["image_id"].tolist()]
#         # random.shuffle(files)
...
#         # samples = random.sample(files, k=self.n_images)
#         samples = files[:5]
#         self.pos_images = [self.load_image2(sample) for sample in samples]
#
#         if not os.path.exists(self.output_dir):
#             os.mkdir(self.output_dir)
    ...
    # def save_models(self):
    #     self.encoder.save_weights(os.path.join(self.model_dir, "encoder_final.h5"))
    #     self.decoder.save_weights(os.path.join(self.model_dir, "decoder_final.h5"))
