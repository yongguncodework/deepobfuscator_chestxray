...
tb = keras.callbacks.TensorBoard(log_dir="./logs/Xray6/utility_classifier_gender/".format(NEW))
'''
...
chkpnt = keras.callbacks.ModelCheckpoint(
    filepath="models/Xray6/utility_classifier_gender.h5",
    monitor="val_auc",
    save_best_only=True,
    save_weights_only=True,
)
...
#     classifier=model.classifier, path="./models/prediction/image_classifier_gender.h5".format(NEW)
# )

a1 = np.unique(np.array(train_generator.classes))
a2 = np.array(train_generator.classes)
...
plt.savefig('Learning_Curve_Xray_img_classifer_diseaseprediction_6.png')
